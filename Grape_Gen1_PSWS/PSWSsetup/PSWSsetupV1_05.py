...
11-25-20  Added a feqw small tweaks JC Gibbons
12-04-20  Fixed default Gridsquare if no Lat/Long/Elev file exists
01-29-21  Added Sxfer directory in file structure
01-26-22  Add directory support for magnetometer / Temp sensor
...
XferDir = PSWSDir + "Sxfer/"
MagTmpDir = PSWSDir + "Smagtmp/"

################################################################
################################################################

print('\n\nGrape Personal Space Weather Station (PSWS) Setup Program Ver 1.04\n\n')
################################################################
################################################################
...
if (path.exists(MagTmpDir)):
    print('Snmagtmp exists ' + MagTmpDir)
else:
...
if (path.exists(NodePath)):
    with open(NodePath, 'r') as NodeFile: # file there - open it
        ...
        print('\nCurrent Node Assignment = '+ OrigSnode)  # display it
...
changeLLE = 0  #indicate no change to LLE file
...
if (path.exists(LLEPath)):
    ...
    with open(LLEPath, 'w') as LLEFile:
        ...
        LLEFile.close()
        changeLLE = changeLLE + 1  #indicate LLE file changed
...
while Done == 0:
    ...
    if Elev == '':
        ...
        if  elvlth >= 2:  #look for 000 char format (allow for - sign
            ...
            changeLLE = changeLLE + 1  # indicate the LLE file changed
            Done = 1
...
if (path.exists(RIDPath)):
    ...
    with open(RIDPath, 'w') as RadioIDFile:
        RadioID = 'N0\n' # create default Radio ID
        RadioIDFile.write(RadioID) #write default Radio ID
        RadioIDFile.close()
...
print('#')
print('# Beacon Now Decoded       ' + Beacon1)
print('#')
print('#######################################')
################################################################
################################################################
