...
#-------------------------------------------------
#-------------------------------------------------
print('WWV Beacon File Re-Processing Program - Ver 0.25')
print('************************************************\n')
...
with open(ToDoPath, 'r') as ToDoFile:
    ...
    while FiletoChk != '':  # keep looping until done
        ...
        with open(ChkFile, "r") as tmpfile:
            ...
            FinalOutFile.write(FirstLine) #write data
            #print('#######################################')
            FinalOutFile.write('#######################################\n') #write data line
            ...
            # create header line of file data - 
                if (nxtline[:3] != "UTC"):
                    linecnt = linecnt + 1
                    #print('HDR:',nxtline)
                    # this will capture and write out the metadata file info if it exists
                    # this will always be before the Header line that starts the data collection
                else:
                    # this is the CSV header already in buffer - rewrite what we want from it
                    #FinalOutFile.write("UTC,Freq,Freq Err,Vpk,dBV(Vpk)\n") # original header in analysisYYMMDD.csv file
            ...
            for line in tmpreader:
                ...
                #writeff.writerow([line[0], line[1], line[2], line[3], line[4]]) #original saved data
        ...
        #print('Moving raw data File to final destination ',)
        #shutil.move(NewFinalFile,ChkFile)
        #os.chmod(ChkFile, mode=0o664)   # set the permissions to 664
...
########
        # Do the plotfile for this file as well
        beacon = SBeacon
        #Lat = '00.000'
        #Long ='-00.000'
        #Elev = '000'
