...
void anal::writeFile()
{
	...
	// Create time astamp for this second (FULL ISO Format timestamp)
        gmtime_r(&now, &File_Time_Now);

	// create the embedded date data image as YYYY-MM-DDTHH:MM:SSZ (ISO Date format)
	//      strftime((char*)FileData,sizeof(FileData),"%Y-%m-%dT%H:%M:%S", &File_Start_Data);
        strftime((char*)FileTimeNow,sizeof(FileTimeNow),"%FT%T", &File_Time_Now);

//        printf("timeStamp Data = %sZ\n",FileTimeNow);
...
// N8OBj 7-9-20 changed time to full ISO format
// Changed /added new .csv fields 
//	header is: fprintf(out, "UTC,Freq,Freq Err,Vpk,dBV(Vpk)\n");

//	fprintf(out, "%02d:%02d:%02dZ, %13.3f, %6.3f, %8.6f, %6.2f\n", //OLD Version
//		tm.tm_hour, tm.tm_min, tm.tm_sec,  //OLD Version
	fprintf(out, "%sZ, %12.3f, %6.3f, %8.6f, %6.2f\n",
		FileTimeNow,
		(wf->rfcarrier() + (wf->USB() ? 1.0 : -1.0) * (frequency + fout)), fout, 
		amp, 20.0 * log10( (amp == 0 ? 1e-6 : amp) ) );

// diag printout
//	printf("%02d:%02d:%02dZ, %13.3f, %6.3f, %8.6f, %6.2f\n", //OLD Version
//                tm.tm_hour, tm.tm_min, tm.tm_sec,  // OLD version

//	printf("%sZ, %12.3f, %6.3f, %8.6f, %6.2f\n",
//                FileTimeNow,
//                (wf->rfcarrier() + (wf->USB() ? 1.0 : -1.0) * (frequency + fout)), fout, 
//                amp, 20.0 * log10( (amp == 0 ? 1e-6 : amp) ) );
       ...
       // Indicate current time on status line
        char TimeNow[10];
        snprintf( TimeNow, sizeof(TimeNow), "%02d:%02d:%02dZ", tm.tm_hour, tm.tm_min, tm.tm_sec );
