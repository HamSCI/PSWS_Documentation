...
yesterdaydate = yesterdaystr # save for use if we need to reswequency any strings
...
Olddate = Olddate.replace('-','') #remove the hyphens

#create file name to check
FileChk = 'analysis'+Olddate+'.csv' #filename format
#-------------------------------------------------
#-------------------------------------------------
#-------------------------------------------------
#-------------------------------------------------
...
#FileChk='analysis200500.csv'  # FLDigi format
#FileChk='analysis200516.csv'  # new format
#FileChk='analysis200528.csv'  # old format
...
### resequencing code goes here
    ...
    fix_timestamps_after_crash.fix_and_write_back(TempFile, yesterdaydate)
...
with open(FinalFile, 'r') as dataFile:
    ...
    # check for previous old header format
    #2020-05-15,N8OBJ Macedonia Ohio EN91fh,LB GPSDO,41.3219273, -81.5047731, 284.5
    if NewHdr == 0:      
        # Try using original FLdigi format w/o info line
        if (Header[0] == "UTC"):
    ...
    NewFinalFileName = UTCDTZ + '_' + node + '_' + radioid + '_' + GridSq + '_FRQ_' + beacon + '.csv'
...
SaveDataFile = SaveDataDir + beacon +'/' + NewFinalFileName
print('Finished Final Format File = ', FinalFile)
